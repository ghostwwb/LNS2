...
bool LNS::run()
{
    ...
    // 如果初始解决方案获取失败且用时小于时间限制
    if (!succ && initial_solution_runtime < time_limit)
    {
        // 判断输入中是否使用lns获取初始解决方案
        if (use_init_lns)
        {
            // 当使用PP获取初始方案失败后，使用initlns获取初始解决方案
    ...
    while (runtime < time_limit && iteration_stats.size() <= num_of_iterations)
    {
        ...
        // 将现有的agent的路径的代价和path记录到neighbor中，并且将path_table中的当前路径删除，以存放新路径
